...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const ssdcoinper_input = ip_parsed.limit;
    ...
    const ssdcoinper_value = Number.parseInt(Number(ssdcoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (ssdcoin per interval):{' '}
                  {mojo_to_ssdcoin_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = ssdcoin_to_mojo(fee_input.value);
