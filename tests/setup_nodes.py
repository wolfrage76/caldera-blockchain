...
from caldera.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from caldera.full_node.full_node_api import FullNodeAPI
from caldera.server.start_farmer import service_kwargs_for_farmer
from caldera.server.start_full_node import service_kwargs_for_full_node
from caldera.server.start_harvester import service_kwargs_for_harvester
from caldera.server.start_introducer import service_kwargs_for_introducer
from caldera.server.start_service import Service
from caldera.server.start_timelord import service_kwargs_for_timelord
from caldera.server.start_wallet import service_kwargs_for_wallet
from caldera.simulator.start_simulator import service_kwargs_for_full_node_simulator
from caldera.timelord.timelord_launcher import kill_processes, spawn_process
...
from caldera.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["xcl_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xcl")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xcl_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xcl")
