...
from caldera.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from caldera.protocols import wallet_protocol
from caldera.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from caldera.protocols.protocol_message_types import ProtocolMessageTypes
from caldera.protocols.wallet_protocol import (
...
from caldera.types.coin_solution import CoinSolution
...
from caldera.util.byte_types import hexstr_to_bytes
from caldera.util.errors import Err, ValidationError
from caldera.util.ints import uint32, uint128
from caldera.util.keychain import Keychain
from caldera.util.lru_cache import LRUCache
from caldera.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from caldera.util.path import mkdir, path_from_root
from caldera.wallet.block_record import HeaderBlockRecord
from caldera.wallet.derivation_record import DerivationRecord
from caldera.wallet.settings.settings_objects import BackupInitialized
from caldera.wallet.transaction_record import TransactionRecord
from caldera.wallet.util.backup_utils import open_backup_file
from caldera.wallet.util.wallet_types import WalletType
from caldera.wallet.wallet_action import WalletAction
from caldera.wallet.wallet_blockchain import ReceiveBlockResult
from caldera.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'caldera keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSCalderaConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSCalderaConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSCalderaConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSCalderaConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSCalderaConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSCalderaConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
