...
def check_keys(new_root: Path) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("ssd_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("ssd_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "ssd_target_address" not in config["farmer"]:
        print(f"Setting the ssd destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "ssd_target_address" not in config["pool"]:
        print(f"Setting the ssd destination address for coinbase reward to {all_targets[0]}")
