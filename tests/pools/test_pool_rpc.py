...
from caldera.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from caldera.protocols import full_node_protocol
from caldera.protocols.full_node_protocol import RespondBlock
from caldera.rpc.rpc_server import start_rpc_server
from caldera.rpc.wallet_rpc_api import WalletRpcApi
from caldera.rpc.wallet_rpc_client import WalletRpcClient
from caldera.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestPoolWalletRpc:
    ...
    def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32, shuil=None) -> bytes32:
        ...
        args.pool_contract_address = encode_puzzle_hash(p2_singleton_puzzle_hash, "txcl")
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_caldera():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_caldera)
