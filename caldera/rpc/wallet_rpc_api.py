...
from caldera.cmds.init_funcs import check_keys
from caldera.consensus.block_rewards import calculate_base_farmer_reward
from caldera.pools.pool_wallet import PoolWallet
from caldera.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from caldera.protocols.protocol_message_types import ProtocolMessageTypes
from caldera.server.outbound_message import NodeType, make_msg
from caldera.simulator.simulator_protocol import FarmNewBlockProtocol
...
from caldera.util.keychain import bytes_to_mnemonic, generate_mnemonic
from caldera.util.path import path_from_root
from caldera.util.ws_message import WsRpcMessage, create_payload_dict
from caldera.wallet.cc_wallet.cc_wallet import CCWallet
from caldera.wallet.derive_keys import master_sk_to_singleton_owner_sk
from caldera.wallet.rl_wallet.rl_wallet import RLWallet
from caldera.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from caldera.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from caldera.wallet.util.trade_utils import trade_record_to_dict
from caldera.wallet.util.transaction_type import TransactionType
from caldera.wallet.util.wallet_types import WalletType
from caldera.wallet.wallet_info import WalletInfo
from caldera.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "caldera_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "caldera_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xcl_target_address")
        pool_target = config["pool"].get("xcl_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from caldera.pools.pool_wallet_info import initial_pool_state_from_dict
