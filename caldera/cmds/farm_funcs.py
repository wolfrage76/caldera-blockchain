...
from caldera.util.misc import format_bytes
from caldera.util.misc import format_minutes
from caldera.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total caldera farmed: {amounts['farmed_amount'] / units['caldera']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['caldera']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['caldera']}")
    ...
    if amounts is None:
        ...
        print("Note: log into your key using 'caldera wallet show' to see rewards for each key")
