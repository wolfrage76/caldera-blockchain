...
from caldera.server.introducer_peers import IntroducerPeers
from caldera.server.outbound_message import Message, NodeType
from caldera.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from caldera.util.errors import Err, ProtocolError
from caldera.util.ints import uint16
from caldera.util.network import is_localhost, is_in_network
...
class CalderaServer:
    def __init__(
        ...
        caldera_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSCalderaConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSCalderaConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.caldera_ca_crt_path, self.caldera_ca_key_path = caldera_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSCalderaConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.caldera_ca_crt_path, self.caldera_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSCalderaConnection] = None
        try:
            connection = WSCalderaConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSCalderaConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSCalderaConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSCalderaConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSCalderaConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSCalderaConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSCalderaConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSCalderaConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSCalderaConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
    ...
    def is_trusted_peer(self, peer: WSCalderaConnection, trusted_peers: Dict) -> bool:
