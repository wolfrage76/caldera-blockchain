...
from caldera.cmds.init_funcs import caldera_init
from caldera.daemon.windows_signal import kill
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Caldera.")
    quit()
...
service_plotter = "caldera plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "caldera": "caldera",
        "caldera_wallet": "start_wallet",
        "caldera_full_node": "start_full_node",
        "caldera_harvester": "start_harvester",
        "caldera_farmer": "start_farmer",
        "caldera_introducer": "start_introducer",
        "caldera_timelord": "start_timelord",
        "caldera_timelord_launcher": "timelord_launcher",
        "caldera_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("caldera daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CALDERA_ROOT
    os.environ["CALDERA_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CALDERA_ROOT: {os.environ['CALDERA_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    caldera_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("caldera_daemon")
