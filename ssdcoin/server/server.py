...
from ssdcoin.util.network import is_localhost, is_in_network
...
class SSDCoinServer:
    def __init__(
        ...
        self.all_connections: Dict[bytes32, WSSSDCoinConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSSSDCoinConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.ssdcoin_ca_crt_path, self.ssdcoin_ca_key_path = ssdcoin_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.ssdcoin_ca_crt_path, self.ssdcoin_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def connection_added(self, connection: WSSSDCoinConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSSSDCoinConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSSSDCoinConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSSSDCoinConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSSSDCoinConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSSSDCoinConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSSSDCoinConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
    ...
    def is_trusted_peer(self, peer: WSSSDCoinConnection, trusted_peers: Dict) -> bool:
